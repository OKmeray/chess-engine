...
def parse_test_data_from_xml_for_best_move(xml_file_path):
    dir_path = os.path.dirname(os.path.realpath(__file__))
    xml_file_path = os.path.join(dir_path, xml_file_path)
    ...
    for position in root.findall('position'):
        fen = position.find('fen').text
        move = position.find('move').text.split()
        mate_in = position.find('matein').text
        data.append({"fen": fen, "move": move, "mate_in": mate_in})

    return data

puzzles = parse_test_data_from_xml_for_best_move("test_best_move_seek_data.xml")

# checkmates


def get_piece_num_by_letter(piece_letter):
    piece = None
    match piece_letter.lower():
        case "p":
            piece = PieceEnum(1)
...
test_data_for_mate_in_one = []
for puzzle in puzzles:
    if int(puzzle["mate_in"]) == 1:
        piece_num = get_piece_num_by_letter(puzzle["move"][0])

        best_move_detail = {
            "piece": piece_num,
            "color": PieceColor.WHITE if puzzle["move"][0] == puzzle["move"][0].upper() else PieceColor.BLACK,
            "square": get_num_by_square_name(puzzle["move"][1]),
            "move": get_num_by_square_name(puzzle["move"][2]),
        }

        test_data_for_mate_in_one.append((puzzle["fen"], best_move_detail))

for puzzle in test_data_for_mate_in_one:
    print(puzzle)
